...
const processSeizureFrequency = (seizures: Seizure[], timeRange: string) => {
  ...
  if (timeRange === "weekly") {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const count = seizures.filter((seizure) =>
        seizure.date.startsWith(monthStr),
      ).length;
      data.push(count);
...
const calculateYearlyStats = (seizures: Seizure[]) => {
  ...
  for (let i = 0; i < 12; i++) {
    const monthStr = `${currentYear}-${String(i + 1).padStart(2, "0")}`;
    monthlyData[monthNames[i]] = seizures.filter((seizure) =>
      seizure.date.startsWith(monthStr),
    ).length;
  }
  ...
  const bestMonthData = Object.entries(monthlyData).reduce(
    (best, [month, count]) => (count > best.count ? { month, count } : best),
    { month: "None", count: 0 },
  );
  const worstMonthData = Object.entries(monthlyData).reduce(
    (worst, [month, count]) => (count < worst.count ? { month, count } : worst),
    { month: "None", count: 0 },
  );
  ...
  if (maxQuarter > minQuarter * 1.5) {
    const peakQuarter = Object.entries(quarterlyBreakdown).find(
      ([_, count]) => count === maxQuarter,
    )?.[0];
    seasonalPattern = `Peak in ${peakQuarter}`;
  }

  // Calculate yearly trend (comparing first half vs second half)
  const firstHalf = monthlyValues
    .slice(0, 6)
    .reduce((sum, count) => sum + count, 0);
  const secondHalf = monthlyValues
    .slice(6, 12)
    .reduce((sum, count) => sum + count, 0);
...
const calculateSeizureStats = (seizures: Seizure[]) => {
  ...
  const mostCommonTrigger = Object.keys(triggerCount).reduce(
    (a, b) => (triggerCount[a] > triggerCount[b] ? a : b),
    "None",
  );
  ...
  for (let i = 1; i < sortedSeizures.length; i++) {
    const prevDate = new Date(
      `${sortedSeizures[i - 1].date} ${sortedSeizures[i - 1].time}`,
    );
    const currDate = new Date(
      `${sortedSeizures[i].date} ${sortedSeizures[i].time}`,
    );
  ...
  const currentGap = Math.floor(
    (now.getTime() - lastSeizureDate.getTime()) / (1000 * 60 * 60 * 24),
  );

  // Calculate seizure frequency (seizures per month)
  const firstSeizure = sortedSeizures[0];
  const firstSeizureDate = new Date(
    `${firstSeizure.date} ${firstSeizure.time}`,
  );
  const totalDays = Math.max(
    1,
    Math.floor(
      (now.getTime() - firstSeizureDate.getTime()) / (1000 * 60 * 60 * 24),
    ),
  );
  const seizuresPerMonth =
    Math.round((seizures.length / totalDays) * 30 * 10) / 10;
...
export default function ReportsScreen() {
  ...
  const triggerData = useMemo(() => processTriggerData(seizures), [seizures]);
  ...
  const renderBarChart = (data: number[], labels: string[]) => {
    ...
    if (isYearly) {
      ...
      const rowHeight = Math.max(
        (plotHeight - rowGap * (totalRows - 1)) / totalRows,
        rowBarHeight,
      );
      const tickCount = 5;
    ...
    return (
      ...
      >
        ...
        <View className="flex-row" style={{ height: chartHeight }}>
          ...
          <View
            className="flex-1"
            style={{ paddingRight: 12, paddingTop, paddingBottom }}
          >
            ...
            <View
              ...
              style={{ height: chartAreaHeight, marginLeft: 8 }}
            >
              {data.map((value, index) => {
                const barHeight = Math.max(
                  (value / maxValue) * (chartAreaHeight - 10),
                  value > 0 ? 6 : 0,
                );
                const barColor =
                  value === 0
        ...
        <View style={{ marginTop: 4, paddingLeft: yAxisWidth }}>
          {/* X-axis baseline */}
          <View
            style={{ height: 2, backgroundColor: "#D1D5DB", marginLeft: 8 }}
          />
          <View className="flex-row" style={{ marginLeft: 8 }}>
            {labels.map((label, index) => (
              <View
                key={index}
                style={{ width: barWidth + barSpacing, alignItems: "center" }}
              >
                <Text
  ...
  const renderTriggerChart = () => {
    if (triggerData.length === 0) {
      return (
        <View
          className="bg-white rounded-xl p-8 shadow-sm items-center justify-center"
          style={{
            elevation: Platform.OS === "android" ? 3 : 0,
            shadowColor: Platform.OS === "ios" ? "#000" : "transparent",
            shadowOffset:
    ...
    const totalTriggers = triggerData.reduce(
          ...
          {triggerData.map((item, index) => {
            ...
            return (
              <View key={index} className="flex-row items-center mb-3">
                <View
                  className={`w-4 h-4 rounded-full mr-3 ${colors[index % colors.length]}`}
                />
  ...
  const renderTimeChart = () => {
    ...
    const totalEvents = timeOfDayData.reduce(
      (sum, item) => sum + item.count,
      0,
    );
    ...
    return (
      ...
      >
        {timeOfDayData.map((item, index) => {
          const percentage =
            totalEvents > 0 ? (item.count / totalEvents) * 100 : 0;
          const colors = [
            "bg-blue-500",
            "bg-green-500",
            "bg-yellow-500",
          ...
          return (
            <View key={index} className="mb-4">
              ...
              <View
                className="h-6 bg-gray-100 rounded overflow-hidden flex-row items-center"
  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 1, marginBottom: Platform.OS === "ios" ? 90 : 80 }}>
        ...
        >
          <View
            className="flex-row bg-white rounded-lg mb-5 shadow-sm overflow-hidden"
            style={{
          ...
          >
            {["weekly", "monthly", "yearly"].map((range) => (
              ...
              >
                <Text
                  className={`text-lg font-medium ${timeRange === range ? "text-white font-bold" : "text-gray-500"}`}
                  style={{
                    fontFamily: Platform.OS === "ios" ? "System" : "Roboto",
            ...
            <View className="mb-6">
              ...
              >
                ...
                <View className="flex-row">
                  <View className="flex-1 items-center">
                    ...
                    >
                      Peak Month ({seizureStats.yearlyStats.bestMonth.count}{" "}
                      seizures)
                    </Text>
                  </View>
                  <View className="flex-1 items-center">
                    ...
                    >
                      Best Month ({seizureStats.yearlyStats.worstMonth.count}{" "}
                      seizures)
                    </Text>
                  </View>
                </View>
              ...
              >
                ...
                <View className="flex-row">
                  ...
                  <View className="flex-1 items-center">
                    <Text
                      className="text-2xl font-bold text-purple-500 mb-1"
                      style={{
                        fontFamily: Platform.OS === "ios" ? "System" : "Roboto",
                      }}
                    >
          ...
          <View className="mb-6">
            ...
            >
              ...
              <View className="flex-row">
                ...
                <View className="flex-1 items-center">
                  <Text
                    className="text-2xl font-bold text-indigo-500 mb-1"
                    style={{
                      fontFamily: Platform.OS === "ios" ? "System" : "Roboto",
                    }}
                  >
            ...
            >
              ...
              <View className="flex-row items-center justify-center">
                <View className="items-center">
                  ...
                  >
                    Based on {medications.filter((m) => m.active).length} active
                    medications
                  </Text>
                </View>
              </View>
