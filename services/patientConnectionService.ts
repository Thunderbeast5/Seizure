...
class PatientConnectionService {
  ...
  async getPatientConnectionRequests(
    patientId: string,
  ): Promise<PatientConnectionRequest[]> {
    try {
      const q = query(
  ...
  async getPatientConnectionRequestsWithDoctorInfo(
    patientId: string,
  ): Promise<DoctorConnectionInfo[]> {
    try {
      ...
      console.error(
        "Error fetching connection requests with doctor info:",
        error,
      );
      throw error;
    }
