...
export const Dashboard: React.FC = () => {
  ...
  const [seizureTrends, setSeizureTrends] = useState<
    { month: string; seizures: number }[]
  >([]);
  const [growthRatePct, setGrowthRatePct] = useState<number | null>(null);
  const [expandedPatientIds, setExpandedPatientIds] = useState<Set<string>>(
    new Set(),
  );
  const [patientDetailsById, setPatientDetailsById] = useState<
    Record<
      string,
      { profile: any; seizures: PatientSeizure[]; medications: any[] }
    >
  >({});
  const [patientDetailsLoading, setPatientDetailsLoading] = useState<
    Record<string, boolean>
  >({});
  const [seizureTypeDist, setSeizureTypeDist] = useState<
    { type: string; value: number }[]
  >([]);
  const [patientSparkById, setPatientSparkById] = useState<
    Record<string, { day: string; count: number }[]>
  >({});
  const [patientSparkMetaById, setPatientSparkMetaById] = useState<
    Record<string, { last7: number; prev7: number; deltaPct: number }>
  >({});
  ...
  const loadPatients = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const assignedPatients = await doctorService.getAssignedPatients(
        user.uid,
      );
      ...
      const enrichedPatients = await Promise.all(
        assignedPatients.map(async (p) => {
          try {
            ...
            const videoCount = seizures.filter(
              (s: PatientSeizure) => !!s.videoUrl,
            ).length;
      ...
      const now = new Date();
      const months: { key: string; label: string }[] = Array.from({
        length: 6,
      }).map((_, idx) => {
      ...
      const counts: Record<string, number> = months.reduce(
        (acc, m) => {
          acc[m.key] = 0;
          return acc;
        },
      ...
      try {
        ...
        setSeizureTypeDist(
          Object.entries(typeCounts).map(([type, value]) => ({ type, value })),
        );

        // Per-patient last 7 days sparkline data
        const dayMsSpark = 24 * 60 * 60 * 1000;
        const nowTsSpark = Date.now();
        const last7Refs: Date[] = Array.from({ length: 7 }).map(
          (_, idx) => new Date(nowTsSpark - (6 - idx) * dayMsSpark),
        );
        const sparkMap: Record<string, { day: string; count: number }[]> = {};
        const sparkMeta: Record<
          string,
          { last7: number; prev7: number; deltaPct: number }
        > = {};
        const prev7StartTs = nowTsSpark - 13 * dayMsSpark; // 14 days window start
        const prev7Refs: Date[] = Array.from({ length: 7 }).map(
          (_, idx) => new Date(prev7StartTs + idx * dayMsSpark),
        );
        assignedPatients.forEach((p, idx) => {
          ...
          const last7Total = last7Refs.reduce(
            (sum, d) => sum + (countsByDay[d.toISOString().slice(0, 10)] || 0),
            0,
          );
          // Build prev7 map
          ...
          const prev7Total = prev7Refs.reduce(
            (sum, d) =>
              sum + (prevCountsByDay[d.toISOString().slice(0, 10)] || 0),
            0,
          );
          const deltaPct =
            prev7Total === 0
        ...
        const last30 = flatSeizures.filter(
          (s) => new Date(s.date) >= last30Start,
        );
        const prev30 = flatSeizures.filter(
          (s) =>
            new Date(s.date) >= prev30Start && new Date(s.date) < last30Start,
        );
        ...
        setGrowthRatePct(
          Number.isFinite(growth) ? Math.round(growth * 10) / 10 : 0,
        );
      } catch {
      ...
      setSeizureTrends(
        months.map((m) => ({ month: m.label, seizures: counts[m.key] || 0 })),
      );
    } catch (error) {
    ...
    (async () => {
      try {
        ...
        if (vapid) {
          ...
          if (token && user?.uid) {
            await setDoc(
              fsDoc(db, "doctorFcmTokens", user.uid),
              {
                token,
        ...
        onForegroundMessage?.((payload) => {
          ...
          const href = data.patientId
            ? `/patient/${data.patientId}?tab=seizures${data.seizureId ? `&seizureId=${encodeURIComponent(data.seizureId)}` : ""}`
            : undefined;
          setToast({
            title: `${data.patientName || "Patient"} logged a ${data.type || "Seizure"}`,
            body: data.severity
              ? `Severity: ${data.severity}`
              : new Date((data.date as string) || Date.now()).toLocaleString(),
            href,
          });
          try {
            playAlert(5000);
  ...
  useEffect(() => {
    ...
    patients.forEach((p) => {
      const unsubscribe = patientDataService.subscribeToPatientSeizures(
        p.userId,
        (seizures) => {
          seizures
          ...
          {patients.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-sm text-gray-500">
                Use the &quot;Search Patients&quot; tab to find and connect with
                patients.
              </p>
            </div>
          ) : (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {patients
                    .slice()
                    .sort((a, b) => {
                      const aDate = a.lastSeizureDate
                        ? new Date(a.lastSeizureDate).getTime()
                        : 0;
                      const bDate = b.lastSeizureDate
                        ? new Date(b.lastSeizureDate).getTime()
                        : 0;
                      return bDate - aDate;
                    })
                    .map((patient) => (
                      <React.Fragment key={patient.userId}>
                        <tr className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <div className="w-40 h-10">
                              <ResponsiveContainer width="100%" height={40}>
                                <AreaChart
                                  data={patientSparkById[patient.userId] || []}
                                >
                                  <defs>
                                    <linearGradient
                                      id={`colorSpark-${patient.userId}`}
                                      x1="0"
                                      y1="0"
                                      x2="0"
                                      y2="1"
                                  ...
                                  <Area
                                    ...
                                    stroke="#0ea5e9"
                                    fillOpacity={1}
                                    fill={`url(#colorSpark-${patient.userId})`}
                                  />
                                </AreaChart>
                              </ResponsiveContainer>
                            </div>
                            <div className="text-xs mt-1 flex items-center gap-2">
                              ...
                              <span className="font-medium">
                                {patientSparkMetaById[patient.userId]?.last7 ??
                                  0}
                              </span>
                              {typeof patientSparkMetaById[patient.userId]
                                ?.deltaPct === "number" && (
                                <span
                                  className={
                                    patientSparkMetaById[patient.userId]
                                      .deltaPct >= 0
                                      ? "text-green-600"
                                      : "text-red-600"
                                  }
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-3">
                            <button
                              onClick={() =>
                                toggleExpandPatient(patient.userId)
                              }
                              ...
                              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                <div className="bg-white rounded-lg shadow p-4">
                                  ...
                                  {patientDetailsLoading[patient.userId] ? (
                                    ...
                                    <div className="text-sm text-gray-600">
                                      {patientDetailsById[patient.userId]
                                        ?.medications?.length ? (
                                        <div>
                                          <div className="mb-2">
                                            Active:{" "}
